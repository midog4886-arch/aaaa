...
## Update 21 - User Permissions System (February 2026)

### New Feature: نظام صلاحيات المستخدمين ✅

تم إضافة نظام صلاحيات متقدم للمستخدمين يسمح بتحديد الصفحات التي يمكن لكل مستخدم الوصول إليها.

#### الميزات المنفذة:

##### 1. حقل الصلاحيات في قاعدة البيانات ✅
- إضافة حقل `permissions` (قائمة) لكل مستخدم
- المدراء يحصلون تلقائياً على جميع الصلاحيات

##### 2. واجهة اختيار الصلاحيات ✅
- قسم جديد في نموذج إضافة/تعديل المستخدم
- 12 صلاحية متاحة:
  - لوحة التحكم، الأعضاء، الأنشطة، المستويات
  - الجدول، الحضور، الفواتير، المخزن
  - المحاسبة، التقارير، الرسائل، الإعدادات
- أزرار "تحديد الكل" و "إلغاء الكل"
- عرض مرئي للصلاحيات المحددة

##### 3. فلترة القائمة الجانبية ✅
- يتم إخفاء الصفحات التي لا يملك المستخدم صلاحية الوصول إليها
- المدراء يرون جميع الصفحات

##### 4. عرض الصلاحيات في جدول المستخدمين ✅
- عمود "الصلاحية" يعرض:
  - "مدير" للمدراء
  - شارات بأسماء الصلاحيات (إذا 3 أو أقل)
  - عدد الصلاحيات (إذا أكثر من 3)
  - "بدون صلاحيات" إذا فارغة

### Files Modified:
- `/app/backend/server.py` - إضافة حقل permissions للنماذج والـ APIs
- `/app/frontend/src/pages/UsersPage.js` - واجهة اختيار الصلاحيات
- `/app/frontend/src/components/Layout.js` - فلترة القائمة الجانبية

### Test Results:
- ✅ إنشاء مستخدم بصلاحيات محدودة (الجدول + الحضور)
- ✅ القائمة الجانبية تعرض فقط الصفحات المسموحة
- ✅ عرض الصلاحيات في جدول المستخدمين
...
- ✅ محاولة الوصول لـ `/dashboard` تعرض صفحة "غير مصرح"
- ✅ القائمة الجانبية تعرض فقط الجدول والحضور

---

## Update 25 - Unauthorized Page (February 2026)

### New Feature: صفحة "غير مصرح بالدخول" ✅

تم إضافة صفحة مخصصة تظهر للمستخدم عند محاولة الوصول لصفحة بدون صلاحية.

#### الميزات:

##### 1. تصميم احترافي ✅
- أيقونة درع مع علامة X حمراء
- خلفية متدرجة من الأحمر للبرتقالي
- بطاقة بيضاء مع ظل

##### 2. معلومات واضحة ✅
- عنوان "غير مصرح بالدخول"
- رسالة توضيحية للمستخدم
- عرض اسم المستخدم الحالي
- عرض قائمة الصلاحيات المتاحة للمستخدم

##### 3. أزرار للتنقل ✅
- زر "الذهاب للصفحة الرئيسية" - ينقل لأول صفحة مسموحة
- زر "تسجيل الخروج" - لتسجيل الخروج

### Files Created:
- `/app/frontend/src/pages/UnauthorizedPage.js` - NEW

### Files Modified:
- `/app/frontend/src/App.js` - إضافة route للصفحة وتحديث ProtectedRoute
...
## Update 28 - Member Code in Invoice Print (December 2025)

### New Feature: إظهار رقم العضوية في طباعة الفاتورة ✅

تم إضافة رقم العضوية بجوار اسم العميل في الفاتورة.

#### الميزات المنفذة:

##### 1. إضافة member_code للفواتير ✅
- إضافة حقل `member_code` لنموذج الفاتورة
- تخزين رقم العضوية عند إنشاء فاتورة جديدة
- جلب رقم العضوية للفواتير القديمة تلقائياً من بيانات العضو

##### 2. عرض رقم العضوية في الفاتورة ✅
- يظهر بجوار الاسم في قسم "بيانات العميل": **الاسم: محمد أحمد (#2620)**
- يظهر في رسالة واتساب
- يظهر في استمارة التسجيل المطبوعة

### Files Modified:
- `/app/backend/server.py`:
  - إضافة `member_code` لنموذج `Invoice`
  - إضافة `member_code` عند إنشاء فاتورة جديدة
  - جلب `member_code` للفواتير القديمة في APIs الجلب
- `/app/frontend/src/pages/InvoicesPage.js`:
  - عرض رقم العضوية في نافذة عرض الفاتورة
  - إضافة رقم العضوية لرسالة واتساب
  - إضافة رقم العضوية في استمارة التسجيل

### Test Results:
- ✅ فاتورة جديدة تحفظ رقم العضوية
- ✅ الفواتير القديمة تعرض رقم العضوية من بيانات العضو
- ✅ رقم العضوية يظهر في نافذة عرض الفاتورة
- ✅ رقم العضوية يظهر في رسالة واتساب

---

## Update 29 - Backend Refactoring Phase 1 (December 2025)

### إعادة هيكلة الـ Backend - المرحلة الأولى ✅

تم البدء بإعادة هيكلة ملف `server.py` الضخم وتقسيمه إلى ملفات منفصلة.

#### ما تم إنجازه:

| الملف | الوصف | الأسطر |
|-------|-------|--------|
| `routes/users.py` | إدارة المستخدمين | ~160 |
| `routes/levels.py` | إدارة المستويات | ~165 |
| `routes/branches.py` | إدارة الفروع | ~100 |
| `routes/activities.py` | إدارة الأنشطة | ~100 |
| `routes/coaches.py` | إدارة المدربين | ~100 |

#### النتائج:
- **حجم server.py الأصلي**: 6,536 سطر
- **حجم server.py الحالي**: 6,114 سطر
- **تم تقليص**: 422 سطر (~6%)

#### الهيكل الجديد:
```
/app/backend/
├── server.py           # الملف الرئيسي (مقلص)
...
│   ├── activities.py   ✅ NEW
│   └── coaches.py      ✅ NEW
├── models/             # (موجود - غير مستخدم بعد)
└── utils/
    └── auth.py         # (موجود - مستخدم جزئياً)
```

#### المسارات المتبقية للنقل (المرحلة 2):
- `/members` (~400 سطر)
- `/invoices` (~500 سطر)
- `/attendance` (~300 سطر)
- `/schedules` (~200 سطر)
- `/notifications` (~200 سطر)
- `/accounting` (~400 سطر)
- وغيرها...

### Test Results:
- ✅ جميع الـ APIs تعمل بشكل صحيح بعد إعادة الهيكلة
- ✅ التطبيق يعمل بدون أخطاء

---

## Update 30 - Schedule Input Component Enhancement (February 2026)

### ميزة جديدة: مكون إدخال الجدول الزمني للتدريب ✅

تم استبدال قائمة الجدول المنسدلة في صفحة الفواتير بمكون جديد يسمح باختيار أيام متعددة وإدخال الوقت يدوياً.

#### ما تم إنجازه:

##### 1. مكون اختيار أيام التدريب ✅
- أزرار تفاعلية لاختيار الأيام: الأحد، الإثنين، الثلاثاء، الأربعاء، الخميس، الجمعة، السبت
- يمكن اختيار أيام متعددة
- تصميم جذاب مع تلوين الأيام المختارة باللون الأزرق

##### 2. حقل إدخال الوقت ✅
- حقل نصي لإدخال وقت التدريب
- مثال: "4:00 م"

##### 3. تنسيق الجدول في الفاتورة ✅
- **الشكل الجديد**: "الأحد، الثلاثاء و الخميس - 4:00 م"
- ترتيب الأيام من الأحد للسبت
- فاصلة بين الأيام و "و" قبل اليوم الأخير
- شرطة قبل الوقت

##### 4. إصلاح الأخطاء البرمجية ✅
- إصلاح استخدام `newInvoice.items` غير الموجود واستبداله بـ `invoiceItems`
- إصلاح استخدام `setNewInvoice` غير الموجود واستبداله بـ `setInvoiceItems`

### الملفات المعدلة:
- `/app/frontend/src/pages/InvoicesPage.js`:
  - سطور 2696-2760: مكون اختيار الأيام والوقت في نموذج الفاتورة
  - سطور 3300-3365: مكون اختيار الأيام والوقت في استمارة التسجيل

### نتائج الاختبار:
- ✅ أزرار اختيار الأيام تعمل بشكل صحيح
- ✅ حقل إدخال الوقت يعمل بشكل صحيح
- ✅ الجدول يُحفظ بالتنسيق الجديد في قاعدة البيانات
- ✅ الجدول يظهر بشكل صحيح في الفاتورة المطبوعة

### مثال على البيانات:
```json
{
  "schedule": "الأحد، الثلاثاء و الخميس - 4:00 م",
  "training_days": ["الأحد", "الثلاثاء", "الخميس"],
...
## Update 33 - Registration Form Data Linking (February 2026)

### ميزة جديدة: ربط بيانات استمارة التسجيل ✅

تم تحديث نظام استمارات التسجيل ليقوم تلقائياً بإرسال البيانات إلى:

#### 1. جدول الأعضاء (Members) ✅
- **إنشاء عضو جديد**: إذا لم يكن رقم الجوال موجوداً
- **ربط الأنشطة**: يتم إضافة النشاط لقائمة أنشطة العضو مع كل التفاصيل:
  - `activity_id` - معرف النشاط
  - `activity_name` - اسم النشاط
  - `level_id` - معرف المستوى (إن وجد)
  - `start_date` - تاريخ البداية
  - `end_date` - تاريخ النهاية
  - `fee` - الرسوم
  - `schedule` - الجدول الزمني
  - `status` - الحالة (active)
  - `source` - المصدر (registration_form)
  - `source_id` - معرف الاستمارة

#### 2. جدول المستويات (Levels) ✅
- يتم إضافة العضو للمستوى المحدد تلقائياً
- يتم حفظ تاريخ انتهاء الاشتراك للإزالة التلقائية

#### 3. نظام الحضور (Attendance) ✅
- النظام يتعرف على العضو ويسمح بتسجيل الحضور
- البحث يعمل برقم العضوية أو الاسم

### التغييرات التقنية:

#### Backend (`server.py`):
- تحديث model `MemberActivity` لإضافة حقول جديدة:
  - `level_id`, `schedule`, `source`, `source_id` (اختيارية)
  - `fee` أصبح اختيارياً (default: 0)
- تحديث endpoint `POST /api/registration-forms` لإرسال البيانات للجداول

### نتائج الاختبار:
```
✅ إنشاء استمارة تسجيل → Form ID: 92d8a9f5-abc5-41ec-8f32-4dccdfb42340
✅ إنشاء عضو جديد → Member Code: 2623
✅ ربط النشاط بالعضو → Activities: [السباحة 2 يوم في الاسبوع]
✅ إضافة للمستوى → Member found in level!
✅ تسجيل الحضور → "تم تسجيل الحضور بنجاح ✓"
```
...
## Update 35 - Sticker Print Layout 1x2 (February 2026)

### تخطيط طباعة الملصقات الجديد 1×2 ✅

**طلب المستخدم**: تغيير نظام طباعة الملصقات من 2×3 (6 خانات) إلى 1×2 (خانتين) بحيث:
- **خانة 1**: كرت العضوية (متغير لكل عضو)
- **خانة 2**: شعار الأكاديمية (ثابت)
- **طباعة الاثنين معاً** على ورقة واحدة

### الملفات المُحدثة:

#### 1. InvoicesPage.js
- `handleStickerPrint()`: يطبع البطاقة + الشعار معاً
- `handleRegFormStickerPrint()`: يطبع البطاقة + الشعار معاً من استمارة التسجيل
- نافذة الحوار: معاينة الخانتين + زر "طباعة الملصقات"

#### 2. MemberCardPage.js
- `handleStickerPrint()`: يطبع البطاقة + الشعار معاً
- نافذة الحوار: معاينة الخانتين + زر "طباعة الملصقات"

#### 3. MemberQRCard.js (بوابة العضو)
- `handleStickerPrint()`: يطبع البطاقة + الشعار معاً
- نافذة الحوار: معاينة الخانتين + زر "طباعة الملصقات"
...
### مميزات التصميم:
- معاينة مباشرة للكرت والشعار قبل الطباعة
- زر واحد لطباعة الاثنين معاً
- حجم ثابت 9سم × 7سم لكل ملصق
...
##### 7. تعديل أسماء الأنشطة ✅ NEW
- **زر تعديل ✏️**: على كل بطاقة نشاط
- **حوار التعديل**: يحتوي على:
  - حقل الأيقونة (emoji)
  - حقل الاسم بالعربي
  - حقل الاسم بالإنجليزي
- **حفظ في localStorage**: التغييرات تبقى محفوظة

##### 8. السحب والإفلات (Drag & Drop) ✅ NEW
...
النموذج الآن **مطابق تماماً** لصفحة الفواتير واستمارات التسجيل:

1. **اختيار النشاط من القائمة** - يظهر الأنشطة من صفحة الأنشطة بدون سعر
2. **بطاقة تفاصيل النشاط** - تظهر بعد اختيار النشاط مع زر حذف
3. **تاريخ البداية والنهاية** - حقول تاريخ كبيرة (h-12)
4. **أيام التدريب بالعربية** - الأحد، الإثنين، الثلاثاء، الأربعاء، الخميس، الجمعة، السبت
5. **الساعة** - حقل رقمي (1-12) مع تحويل تلقائي لـ "X:00 م" (مساءً)
6. **المستوى** - قائمة متسلسلة مع شريط التعبئة (نشاط ← وقت ← مستوى)

---

## Update - Invoice & Registration Time Input (December 2025)

### إدخال الوقت كرقم وتحويله تلقائياً ✅

#### ما تم إنجازه:

في جميع النماذج (الفواتير، استمارات التسجيل، الأعضاء):

1. **حقل الساعة الجديد**:
   - إدخال رقم فقط (مثال: 4)
   - تحويل تلقائي لـ "4:00 م" (مساءً افتراضياً)
   - يظهر الوقت المُحوّل أسفل الحقل

2. **ربط صفحة الجدول بصفحة الأنشطة**:
   - فلتر النشاط يعرض الأنشطة من صفحة الأنشطة مباشرة
   - القائمة تتحدث تلقائياً مع أي تغيير في الأنشطة

#### الملفات المُحدثة:
- `/app/frontend/src/pages/InvoicesPage.js` - حقل الساعة الرقمي للفواتير واستمارات التسجيل
- `/app/frontend/src/pages/MembersPage.js` - حقل الساعة الرقمي للأعضاء
- `/app/frontend/src/pages/SchedulePage.js` - ربط فلتر النشاط بصفحة الأنشطة
...
## Update - Ads Expiry Notifications (February 2026)

### نظام إشعارات انتهاء الإعلانات ✅

#### ما تم إنجازه:

##### 1. APIs جديدة:
- `POST /api/notifications/check-ads-expiry` - فحص الإعلانات وإنشاء إشعارات
- `GET /api/notifications/ads-status` - حالة جميع الإعلانات (منتهية، تنتهي قريباً، نشطة)

##### 2. أنواع الإشعارات:
- **🔔 إعلان ينتهي قريباً** (قبل 3 أيام من الانتهاء)
- **⚠️ إعلان منتهي** (بعد انتهاء تاريخ الصلاحية)

##### 3. واجهة صفحة الإعلانات:
- قسم جديد يعرض **الإعلانات المنتهية** (باللون الأحمر)
- قسم جديد يعرض **الإعلانات التي تنتهي قريباً** (باللون البرتقالي)
- زر "تمديد" أو "تعديل" مباشر من كل إعلان
- زر **فحص الإعلانات المنتهية وإرسال إشعارات**

##### 4. الإشعارات في لوحة التحكم:
- تظهر في قائمة الإشعارات الرئيسية (🔔)
- رابط مباشر لصفحة الإعلانات
- تصنيف حسب النوع (ad_expired, ad_expiring_soon)
...
#### اختبار الـ PWA:
```
SW Status: Service Worker registered: https://academy-manager-30.preview.emergentagent.com/
```
