...
- **Attendance Tracking:**
    ...
    - **Coach Attendance:** Tracking check-in/check-out times, work hours calculation, and monthly reports. Coaches have `contract_type` (full_time/part_time) and `monthly_work_days` fields. Absent days are calculated automatically as `monthly_work_days - present_days - leave_days` (min 0) instead of counting explicit "absent" records.
