...
- **Member Self-Service Profile:** Members can visit `/member-profile` from the portal nav (or by tapping their name in the header) to view their basic info (name, member code, phone, date of birth — read-only) and edit their email, address, emergency contact, and profile picture. The avatar is stored as a base64 data URL in `members.photo` (≤2MB, validated by `routes/member_portal.py::update_member_profile`, mirroring the coach-photo pattern). The endpoint is `PUT /api/member-portal/profile` and accepts a partial payload, so avatar-only saves and contact-only saves both work. **Pending change-request badges:** when a member files a change request for a locked field (name / phone / date of birth) via `POST /api/member-portal/profile/change-request`, the profile page shows a "Pending review" badge with the requested value next to that field. The frontend reads `GET /api/member-portal/profile/change-requests`, which returns at most one pending entry per field by filtering `db.messages` for `kind="profile_change_request"`, `sender_type="member"`, and `read_by_admin=False`. Once an admin opens that member's conversation in the inbox (which sets `read_by_admin=True`), the badge clears on the next page visit. **Apply confirmation:** when an admin clicks "Apply change", `apply_profile_change_request` (in `routes/members.py`) updates the member record, then appends an admin->member message in the same thread with field-aware bilingual copy (e.g. `تم تحديث رقم الجوال إلى 05XXXXXXXX / Your phone has been updated to 05XXXXXXXX`) and fires a push notification via `send_message_push` (lazy-imported from `routes/messages.py`). The confirmation message has `read_by_member=False` so it counts toward the member's unread badge until they open the messages tab. Push failures are swallowed so the profile update is never rolled back. Field labels live in `_PROFILE_CHANGE_FIELD_LABELS` and cover the three supported fields (name / phone / date of birth).
...
- **Attendance Tracking:**
    ...
    - **Member Photos in Attendance Reports:** The shared `frontend/src/components/MemberAvatar.jsx` component (sizes `xs`/`sm`/`md`/`lg`, photo + initials fallback) is used in the AttendancePage Reports tab `member_stats` table. The activity attendance report (`get_activity_attendance_report`) and the per-member aggregation in the export endpoint enrich each `member_stats` entry with `member_photo` (fetched from the `members` collection in batch, falling back to the photo stored on the attendance record). The `/api/export/attendance` endpoint embeds member photo thumbnails into both XLSX (≈36×36 px in the new "صورة" column via `openpyxl.drawing.image.Image`) and PDF (10×10 mm via `reportlab.platypus.Image`) outputs. Photos are stored as base64 data URLs (`data:image/...;base64,...`) and decoded by the shared `_decode_photo_data_url` helper in `backend/server.py`; missing/malformed photos render as an empty cell. Note: the `_get_reportlab` helper now wraps `getSampleStyleSheet` in `staticmethod(...)` to avoid Python binding plain functions stored on the dummy class instance — without this the PDF export raises `TypeError: getSampleStyleSheet() takes 0 positional arguments but 1 was given`.
