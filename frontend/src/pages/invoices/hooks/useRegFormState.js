...
export const useRegFormState = ({
  ...
  const [regFormPaymentMethod, setRegFormPaymentMethod] = useState('card');
  ...
  const handleSendRegFormWhatsApp = (form) => {
    ...
    const itemsList = form.items?.map(item => {
      ...
      const startLine = !isNaN(startTs) ? `\n  ▶️ تاريخ البداية: ${new Date(startTs).toLocaleDateString('ar-SA')}` : '';
