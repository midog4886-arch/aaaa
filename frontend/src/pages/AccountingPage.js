...
export default function AccountingPage() {
  ...
  const renderReportsTab = () => (
    <div className="space-y-6">
      ...
      {/* Revenue Summary Card - NEW */}
      <div className="mt-6 border-2 border-green-200 rounded-lg p-6 bg-gradient-to-br from-green-50 to-white">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-bold text-green-800">💰 ملخص الإيرادات</h3>
          <Button 
            onClick={() => {
              // Print revenue summary
              const totalRevenue = salesReport?.summary?.total_amount || 0;
              const tabyTamaraRevenue = salesReport?.by_payment_method?.['تابي']?.total || 0 + 
                                        salesReport?.by_payment_method?.['تمارة']?.total || 0;
              const tabyTamaraFees = tabyTamaraRevenue * 0.075;
              const tempExpensesTotal = tempExpenses.reduce((sum, e) => sum + parseFloat(e.amount || 0), 0);
              const netRevenue = totalRevenue - tabyTamaraFees - tempExpensesTotal;
              ...
              const printContent = `
                ...
                <head>
                  <meta charset="UTF-8">
                  <title>ملخص الإيرادات</title>
                  <style>
                    body { font-family: Arial, sans-serif; padding: 20px; }
                    .header { text-align: center; margin-bottom: 30px; border-bottom: 2px solid #333; padding-bottom: 15px; }
                    .card { border: 1px solid #ddd; border-radius: 8px; padding: 15px; margin-bottom: 15px; }
                    .card-green { background: #f0fdf4; border-color: #22c55e; }
                    .card-purple { background: #faf5ff; border-color: #a855f7; }
                    .card-red { background: #fef2f2; border-color: #ef4444; }
                    .card-blue { background: #eff6ff; border-color: #3b82f6; }
                ...
                <body>
                  ...
                  <div class="card card-green">
                    <div class="label">💵 إجمالي الإيرادات (المبلغ الكامل)</div>
                    <div class="amount" style="color: #16a34a;">${totalRevenue.toLocaleString()} ر.س</div>
                  </div>
                  
                  <div class="card card-purple">
                    <div class="label">📱 تابي وتمارة</div>
                    <table>
                      <tr><td>المبلغ</td><td>${tabyTamaraRevenue.toLocaleString()} ر.س</td></tr>
                      <tr><td>رسوم (7.50%)</td><td style="color: red;">- ${tabyTamaraFees.toLocaleString()} ر.س</td></tr>
                      <tr class="total-row"><td>الصافي</td><td>${(tabyTamaraRevenue - tabyTamaraFees).toLocaleString()} ر.س</td></tr>
                    </table>
                  </div>
                  ...
                  <div class="card card-red">
                    ...
                    <table>
                      ...
                      ${tempExpenses.map(e => `<tr><td>${e.description}</td><td>${parseFloat(e.amount).toLocaleString()} ر.س</td></tr>`).join('')}
                      <tr class="total-row"><td>الإجمالي</td><td>${tempExpensesTotal.toLocaleString()} ر.س</td></tr>
                    </table>
                  </div>
                  ` : ''}
                  
                  <div class="card card-blue">
                    <div class="label">💎 صافي الإيرادات النهائي</div>
                    <div class="amount" style="color: #2563eb;">${netRevenue.toLocaleString()} ر.س</div>
                  </div>
                  
                  <div style="margin-top: 30px; text-align: center; color: #999; font-size: 12px;">
                    تم الطباعة: ${new Date().toLocaleString('ar-SA')}
                  </div>
                </body>
                </html>
              `;
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white border-2 border-green-300 rounded-xl p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-600 mb-1">💵 إجمالي الإيرادات (المبلغ الكامل)</p>
                  <p className="text-3xl font-bold text-green-600">
                    {(salesReport.summary?.total_amount || 0).toLocaleString()} ر.س
                  </p>
                </div>
            ...
            <div className="bg-white border-2 border-purple-300 rounded-xl p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <p className="text-gray-600 font-semibold">📱 تابي وتمارة (خصم 7.50%)</p>
                <span className="text-purple-600 text-sm bg-purple-100 px-2 py-1 rounded">BNPL</span>
              </div>
              {(() => {
                ...
                return (
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span>تابي:</span>
                      <span>{tabyAmount.toLocaleString()} ر.س</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>تمارة:</span>
                      <span>{tamaraAmount.toLocaleString()} ر.س</span>
                    </div>
                    <div className="border-t pt-2 flex justify-between">
                      <span>الإجمالي:</span>
                      <span className="font-bold">{totalBNPL.toLocaleString()} ر.س</span>
                    </div>
                    <div className="flex justify-between text-red-600">
                      <span>رسوم (7.50%):</span>
                      <span>- {fees.toLocaleString()} ر.س</span>
                    </div>
                    <div className="border-t pt-2 flex justify-between text-lg font-bold text-purple-700">
                      <span>الصافي:</span>
                      <span>{netBNPL.toLocaleString()} ر.س</span>
                    </div>
                  </div>
                );
              })()}
            ...
            {/* Temporary Expenses Card */}
            <div className="bg-white border-2 border-red-300 rounded-xl p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <p className="text-gray-600 font-semibold">📤 المصروفات (داخل الصفحة فقط)</p>
                <span className="text-red-600 text-xs bg-red-100 px-2 py-1 rounded">لا تُحفظ</span>
              </div>
              ...
              {tempExpenses.length > 0 ? (
                <div className="space-y-2">
                  {tempExpenses.map((expense, idx) => (
                    <div key={expense.id} className="flex items-center justify-between bg-red-50 p-2 rounded">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-red-600">{parseFloat(expense.amount).toLocaleString()} ر.س</span>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setTempExpenses(tempExpenses.filter(e => e.id !== expense.id))}
                ...
                <p className="text-center text-gray-400 py-2">لا توجد مصروفات مضافة</p>
              )}
            </div>
            
            {/* Net Revenue Card */}
            <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl p-5 text-white shadow-lg">
              <p className="text-blue-100 mb-1">💎 صافي الإيرادات النهائي</p>
              <p className="text-4xl font-bold">
                {(() => {
                  const total = salesReport.summary?.total_amount || 0;
                  const tabyTamaraTotal = (salesReport.by_payment_method?.['تابي']?.total || 0) + 
                                          (salesReport.by_payment_method?.['تمارة']?.total || 0);
                  const fees = tabyTamaraTotal * 0.075;
                  const expensesTotal = tempExpenses.reduce((sum, e) => sum + parseFloat(e.amount || 0), 0);
                  return (total - fees - expensesTotal).toLocaleString();
                })()} ر.س
              </p>
              <p className="text-blue-200 text-sm mt-2">
                = الإجمالي - رسوم تابي/تمارة - المصروفات
              </p>
            </div>
          </div>
        )}
