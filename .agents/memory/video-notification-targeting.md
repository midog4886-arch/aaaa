...
- **Data caveat:** admins type the activity/hour into the free-text video title (e.g. "السباحة الساعة 5") and historically left the structured `activity_id` empty (~87/99 videos). Accurate per-activity targeting therefore depends on admins actually picking the activity in the upload form.
- **2026-06 decision (user override):** activity selection on the video upload form is now OPTIONAL, not required. The user explicitly asked not to force linking a video to a specific activity ("لا اريد الربط بنشاط معين"). The form has a "بدون نشاط (كل الأعضاء)" option; choosing it sends the video/notification branch-wide (or to everyone if no branch). Do NOT re-add the required-activity validation. **Why:** the user prefers branch-wide reach over enforced per-activity scoping.
